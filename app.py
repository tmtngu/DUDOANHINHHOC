...
# BỘ CSS SANG XỊN MỊN GỐC CỦA BẠN
st.markdown("""
    <style>
...
# TẢI BỘ NÀO AI (.KERAS) TỰ ĐỘNG FIX LỖI INPUTLAYER
@st.cache_resource
def load_keras_model():
    try:
        ...
        # Lớp vá thông minh để đọc file .keras tạo từ Keras 3 trên môi trường cũ hơn nếu cần
        ...
        return None, f"⚠️ Đang cấu hình hệ thống: {e}"

# Khởi tạo mô hình trước khi nạp giao diện sidebar
ai_model, status_msg = load_keras_model()

# CẤU HÌNH THANH SIDEBAR TRÁI
with st.sidebar:
...
with tab_predict:
    if not CANVAS_AVAILABLE:
        st.error("⏳ Hệ thống đang khởi tạo môi trường vẽ. Vui lòng bấm 'Manage App' -> 'Reboot App' hoặc đợi một chút nhé!")
    else:
        ...
        with col1:
            st.markdown("<div class='section-header-center'><div class='custom-green-tag'>🎨 Khung vẽ tương tác (Nét Đen - Nền Trắng)</div></div>", unsafe_allow_html=True)
            ...
            if canvas_result.image_data is not None:
                ...
                if np.mean(img_gray) < 254:
                    if ai_model is not None:
                        # Tiền xử lý ảnh chuẩn đầu vào ảnh xám 64x64 giống hệt trên Colab
                        img_resized = cv2.resize(img_gray, (64, 64))
                        img_input = np.expand_dims(img_resized, axis=(0, -1))
                        img_input = img_input.astype(np.float32)
                        
                        # Dự đoán xác suất từ file Keras gốc
                    ...
                    if confidence >= 80:
                        st.success(f"🌟 **Cực kỳ xuất sắc!** Thói quen vẽ và cấu trúc nét của ông bạn đang đạt chuẩn đặc trưng rất rõ ràng của hình **{pred_label}**.")
                    elif confidence >= 50:
                        st.info(f"👍 **Xếp loại Nhận diện Khá!** Hệ thống dự đoán đây là hình **{pred_label}**.")
                    elif confidence >= 25:
                        st.warning(f"⚠️ **Xếp loại Nhận diện Trung bình.** AI đang phân vân khá nhiều nhưng nghiêng về hình **{pred_label}**.")
                    else:
                        st.error(f"🚨 **Báo động đỏ!** Nét vẽ quá rối rắm khiến mạng CNN không thể phân tích rõ đặc trưng.")
                else:
                    st.info("🖌️ **Hướng dẫn:** Đặt bút quẹt một hình bất kỳ vào ô trắng bên trái, mạng CNN sẽ chẩn đoán Realtime.")
